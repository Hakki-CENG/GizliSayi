...
export default function GizliSayiOyunu() {
  ...
  // --- 1. VERİ TAKİBİ VE BEYAZ EKRAN KORUMASI ---
  useEffect(() => {
    let unsub = () => {};
    if (roomName && (['lobby', 'game', 'selectNumber', 'results'].includes(screen))) {
      unsub = onSnapshot(doc(db, "rooms", roomName), (snapshot) => {
        if (snapshot.exists()) {
          ...
          setGameState(roomData.gameState || {}); // Hata koruması: boş obje
          
          if (roomData.host === myPlayerId) setIsHost(true);

          // Faz Geçişleri (Crash korumalı)
          const phase = roomData.gameState?.phase;
          if (phase === 'selectingNumbers' && (screen === 'lobby' || screen === 'results')) setScreen('selectNumber');
          if (phase === 'playing' && screen === 'selectNumber') setScreen('game');
          if (phase === 'finished' && screen === 'game') setTimeout(() => setScreen('results'), 3500);
        } else {
          // Oda silindiyse menüye güvenli dönüş
          setScreen('menu');
        }
      }, (error) => {
    ...
    return () => unsub();
  }, [roomName, screen, myPlayerId]);

  // --- 2. KRONOMETRE BUG TAMİRİ ---
  useEffect(() => {
    let intervalId = null;

    if (gameState?.phase === 'playing' && screen === 'game' && gameState?.turnStartTime) {
      intervalId = setInterval(() => {
        const now = Date.now();
        const start = gameState.turnStartTime;
        const elapsed = now - start;
        
        // 120 saniye bug'ını engellemek için Math.min ve Math.max kullanımı
        const remaining = Math.max(0, Math.min(15, Math.ceil((15000 - elapsed) / 1000)));
      ...
      }, 500); // 500ms daha hassas takip sağlar
    }
  ...
  const applyTimeoutPenalty = async () => {
    if (!roomName || !gameState?.turnOrder) return;
    try {
      const currentPlayerId = gameState.turnOrder[gameState.turnIndex];
      const updatedPlayers = players.map(p => p.id === currentPlayerId ? { ...p, score: (p.score || 0) - 20 } : p);
      ...
      console.error("Penaltı hatası:", e);
    }
  };
  ...
  const createRoom = async () => {
    ...
    try {
      await setDoc(doc(db, "rooms", roomName), {
        ...
        gameState: { phase: 'lobby', clickedNumbers: [], foundPlayers: [] }, createdAt: Date.now()
      });
      setScreen('lobby');
  ...
  const submitSecretNumber = async () => {
    if (secretNumber === null || secretNumber < 1 || secretNumber > (currentRoom?.maxNumber || 100)) {
    ...
    try {
      ...
      for (const player of players) {
        ...
        if (secSnap.exists() && secSnap.data().number === num) {
          ...
          const earnedPoints = Math.round(BASE_POINTS * Math.pow(SCORE_RATIO, (gameState?.foundPlayers?.length || 0)));
          newPlayers = newPlayers.map(p => p.id === player.id ? { ...p, score: (p.score || 0) + earnedPoints } : p);
          newPlayers = newPlayers.map(p => p.id === myPlayerId ? { ...p, score: (p.score || 0) + FINDER_PENALTY } : p);
        }
      }
      ...
      const allPossibleNumbers = Array.from({length: currentRoom?.maxNumber || 60}, (_, i) => i + 1);
      ...
      await updateDoc(doc(db, "rooms", roomName), {
        ...
        "gameState.turnIndex": nextTurnIndex,
        "gameState.turnStartTime": Date.now(),
        "gameState.phase": isNormalEnd ? 'finished' : 'playing',
        players: newPlayers
      });
    } catch (e) { console.error("Tıklama hatası:", e); }
  };
  ...
  // --- UI GÖRÜNÜMLERİ (Optional Chaining ile güçlendirilmiş) ---
  ...
  if (screen === 'lobby') return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="max-w-xl w-full bg-white p-10 rounded-[3rem] shadow-xl text-center border-b-8 border-slate-200">
        ...
        <h2 className="text-3xl font-black mb-1">{currentRoom?.name || "Oda Yükleniyor..."}</h2>
        <p className="text-slate-400 font-bold uppercase text-[10px] mb-8 tracking-widest">Maksimum {currentRoom?.maxPlayers || 0} Kişi</p>
    ...
    return (
      <div className="min-h-screen bg-indigo-600 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white p-10 rounded-[3rem] text-center shadow-2xl">
          ...
          <p className="text-slate-400 font-bold mb-8 uppercase text-[10px]">1 - {currentRoom?.maxNumber || 60} arası bir sayı seç.</p>
          {hasSelected ? (
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-4 md:p-8 flex flex-col lg:flex-row gap-6">
        {celebration && (
          <div className="fixed inset-0 bg-slate-900/95 flex items-center justify-center z-50 p-4">
            <div className="bg-white p-10 rounded-[3rem] text-center shadow-2xl border-b-8 border-yellow-400 scale-110">
                ...
                <p className="font-bold text-slate-500">Bulan: {celebration.finder}</p>
        ...
        <div className="flex-1 max-w-5xl mx-auto w-full">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div className={`p-6 rounded-3xl text-white shadow-xl flex justify-between items-center md:col-span-2 border-b-8 transition-all duration-300 ${isMyTurn ? 'bg-pink-600 border-pink-800 scale-105' : 'bg-slate-800 border-slate-900 opacity-80'}`}>
              <div>
                  ...
                  <div className="text-2xl font-black truncate max-w-[150px]">
          ...
          <div className="grid grid-cols-5 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-2 bg-slate-900 p-4 rounded-[2rem] border-2 border-slate-800 shadow-inner overflow-hidden">
            {Array.from({length: currentRoom?.maxNumber || 60}, (_, i) => i + 1).map(num => {
